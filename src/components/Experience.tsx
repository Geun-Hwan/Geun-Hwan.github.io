...
const Experience: React.FC<Props> = ({ resumeExperience, resumeBasicInfo }) => {
  if (!resumeExperience || !resumeBasicInfo) return null;

  const sectionName = resumeBasicInfo.section_name.experience;

  const work = resumeExperience.map((workItem, i) => {
    const mainTech = workItem.mainTech.map((tech, index) => (
      <Badge pill className="main-badge mr-2 mb-2" key={index}>
        {tech}
      </Badge>
    ));

    const tech = workItem.technologies.map((tech, index) => (
